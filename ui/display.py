...
def show_message(message):
    """Show a message using pygame's message box."""
    pygame.font.init()
    font = pygame.font.Font(None, 36)
    text = font.render(message, True, (255, 255, 255))
    text_rect = text.get_rect(center=(400, 300))
    
    # Create a separate surface for the message box
    message_surface = pygame.Surface((800, 600))
    message_surface.fill((0, 0, 0))
    message_surface.blit(text, text_rect)
    
    # Blit the message surface onto the main display
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    pygame.quit()
                    sys.exit()
    # No need to quit the display, just update the main display
...
class Display(object):
    def __init__(self, game=None, env=None, interactive=False, debug_mode=False, policies=None, test=False):
        ...
        # Reduced hexagon size for 13-inch screen
        self.hexagon_side_len = 60.0  # Reduced from 82.25
        self.hexagon_height = int(2 * self.hexagon_side_len)
        self.hexagon_width = int(np.sqrt(3) * self.hexagon_side_len)

        # Remove tile spacing to ensure perfect fit
        self.tile_spacing = 0  # No spacing between tiles
        ...
        # Reduced token and building sizes
        self.token_dim = 40  # Reduced from 55
        self.building_scale = 0.3  # Reduced from 0.4
        self.building_height = int(151 * self.building_scale)
        self.building_width = int(129 * self.building_scale)

        self.dice_scale = 0.4  # Reduced from 0.5
        self.dice_height = int(111 * self.dice_scale)
        self.dice_width = int(109 * self.dice_scale)

        # Adjusted screen dimensions for 13-inch MacBook Pro
        if self.debug_mode:
            self.screen_width, self.screen_height = 1600, 900  # Reduced from 2200, 1100
        else:
            self.screen_width, self.screen_height = 1280, 800  # Reduced from 1735, 1100

        # Adjusted starting position for first tile
        self.first_tile_pos = (200, 250)  # Adjusted from (250, 300)
        ...
        for i in range(1, 19):
            for j in range(i):
                ...
                if TL is not None and TL == j:
                    self.tile_pos[i] = (self.tile_pos[j][0] + (self.hexagon_side_len + self.tile_spacing) *(np.sqrt(3)/2.0),
                                        self.tile_pos[j][1] + (3.0/2.0)*(self.hexagon_side_len + self.tile_spacing))
                    self.scaled_tile_pos[i] = (self.scaled_tile_pos[j][0] + (self.outer_hexagon_side_len + self.tile_spacing) * (np.sqrt(3) / 2.0),
                                        self.scaled_tile_pos[j][1] + (3.0 / 2.0) * (self.outer_hexagon_side_len + self.tile_spacing))
                    break
                    ...
                    self.tile_pos[i] = (self.tile_pos[j][0] + self.hexagon_width + self.tile_spacing,
                                        self.tile_pos[j][1])
                    self.scaled_tile_pos[i] = (self.scaled_tile_pos[j][0] + self.outer_hexagon_width + self.tile_spacing,
        ...
        self.development_card_scale = 0.25  # Reduced from 0.28
        ...
        self.played_development_cards_properties = {
            "start": [850, 85],  # Adjusted from 950, 85
            "max_in_row": 4,
            "x_shift": 80,  # Reduced from 90
            "y_shift": 35
        }

        self.play_development_cards_start_pos = {
            "start": [700, 495],  # Adjusted from 800, 495
            "x_shift": 100,  # Reduced from 110
        }
        ...
        pygame.init()
        pygame.font.init()
        self.screen = pygame.display.set_mode((self.screen_width, self.screen_height), pygame.FULLSCREEN)  # Enable full-screen mode
        pygame.display.set_caption("Settlers of Catan RL environment")
        self.BACKGROUND_COLOUR = (25, 105, 158)
        self.font = pygame.font.SysFont('Arial', 20)

        # Scale top menu to full screen width and increased height
        self.top_menu = pygame.transform.scale(pygame.image.load(os.path.join(*self.image_path, "menu/top_header.png")), 
                                             (self.screen_width, 220))  # Increased height to 220
        # self.action_menu = pygame.transform.scale(pygame.image.load(os.path.join(*self.image_path, "menu/action_menu.png")), (300, 200))  # Adjusted to be proportionate
        self.building_cost_menu = pygame.transform.scale(pygame.image.load(os.path.join(*self.image_path, "menu/building_cost_menu.png")), (300, 200))  # Adjusted to be proportionate
        self.tick_image = pygame.transform.scale(pygame.image.load(os.path.join(*self.image_path, "menu/tick.png")), (50, 50))  # Adjusted to be proportionate

        self.ai_play_image = pygame.transform.scale(
            pygame.image.load(os.path.join(*self.image_path, "menu/ai_go.png")), (100, 100))  # Reduced from 120, 120

        self.top_menu_font = pygame.font.SysFont('Arial', 35)  # Reduced from 45
        self.count_font = pygame.font.SysFont('Arial', 16)  # Reduced from 18
        self.thinking_font = pygame.font.SysFont('Arial', 30)  # Reduced from 36
        ...
        self.ROAD_WIDTH = 10  # Reduced from 15
        self.CORNER_RADIUS = 4  # Reduced from 5

        # Adjusted resource box dimensions
        development_card_res_box_width = 35
        development_card_res_box_height = 30
        self.development_card_res_boxes = {
            Resource.Wood: [1000, 456, development_card_res_box_width, development_card_res_box_height],
            Resource.Brick: [1046, 456, development_card_res_box_width, development_card_res_box_height],
            Resource.Sheep: [1092, 456, development_card_res_box_width, development_card_res_box_height],
            Resource.Wheat: [1138, 456, development_card_res_box_width, development_card_res_box_height],
            Resource.Ore: [1184, 456, development_card_res_box_width, development_card_res_box_height]
        }

        # Adjusted harbour trade boxes
        self.harbour_trade_res_boxes = {
            Resource.Wood: [773, 767, development_card_res_box_width, development_card_res_box_height],
            Resource.Brick: [843, 767, development_card_res_box_width, development_card_res_box_height],
            Resource.Sheep: [913, 767, development_card_res_box_width, development_card_res_box_height],
            Resource.Wheat: [983, 767, development_card_res_box_width, development_card_res_box_height],
            Resource.Ore: [1053, 767, development_card_res_box_width, development_card_res_box_height]
        }

        # Adjusted harbour receive boxes
        self.harbour_receive_res_boxes = {
            Resource.Wood: [773, 840, development_card_res_box_width, development_card_res_box_height],
            Resource.Brick: [843, 840, development_card_res_box_width, development_card_res_box_height],
            Resource.Sheep: [913, 840, development_card_res_box_width, development_card_res_box_height],
            Resource.Wheat: [983, 840, development_card_res_box_width, development_card_res_box_height],
            Resource.Ore: [1053, 840, development_card_res_box_width, development_card_res_box_height]
        }

        # Adjusted harbour circle positions
        harbour_circle_radius = 25
        # Center the circles on the harbor images by accounting for image dimensions
        self.harbour_select_circles = {
            Resource.Wood: [(773 + self.token_dim//2, 670 + self.token_dim//2), harbour_circle_radius],
            Resource.Brick: [(843 + self.token_dim//2, 670 + self.token_dim//2), harbour_circle_radius],
            Resource.Sheep: [(913 + self.token_dim//2, 670 + self.token_dim//2), harbour_circle_radius],
            Resource.Wheat: [(983 + self.token_dim//2, 670 + self.token_dim//2), harbour_circle_radius],
            Resource.Ore: [(1053 + self.token_dim//2, 670 + self.token_dim//2), harbour_circle_radius],
            None: [(1123 + self.token_dim//2, 670 + self.token_dim//2), harbour_circle_radius]
        }

        # Adjusted trading semi-circle positions
        semi_circle_scale = 0.45  # Reduced from 0.5
        ...
        self.trade_player_resource_boxes = {
            Resource.Wood: [1100, 720, development_card_res_box_width, development_card_res_box_height],
            Resource.Brick: [1150, 720, development_card_res_box_width, development_card_res_box_height],
            Resource.Sheep: [1200, 720, development_card_res_box_width, development_card_res_box_height],
            Resource.Wheat: [1250, 720, development_card_res_box_width, development_card_res_box_height],
            Resource.Ore: [1300, 720, development_card_res_box_width, development_card_res_box_height]
        }

        self.trade_player_active_boxes = [
            [1100, 750, development_card_res_box_width, development_card_res_box_height],
            [1150, 750, development_card_res_box_width, development_card_res_box_height],
            [1200, 750, development_card_res_box_width, development_card_res_box_height],
            [1250, 750, development_card_res_box_width, development_card_res_box_height]
        ]

        self.receive_player_resource_boxes = {
            Resource.Wood: [1100, 800, development_card_res_box_width, development_card_res_box_height],
            Resource.Brick: [1150, 800, development_card_res_box_width, development_card_res_box_height],
            Resource.Sheep: [1200, 800, development_card_res_box_width, development_card_res_box_height],
            Resource.Wheat: [1250, 800, development_card_res_box_width, development_card_res_box_height],
            Resource.Ore: [1300, 800, development_card_res_box_width, development_card_res_box_height]
        }

        self.receive_player_active_boxes = [
            [1100, 830, development_card_res_box_width, development_card_res_box_height],
            [1150, 830, development_card_res_box_width, development_card_res_box_height],
            [1200, 830, development_card_res_box_width, development_card_res_box_height],
            [1250, 830, development_card_res_box_width, development_card_res_box_height]
        ]

        self.player_box_width = 30  # Reduced from 37
        self.player_box_height = 30  # Reduced from 37
        self.player_boxes = {
            PlayerId.White: [1020, 730, self.player_box_width, self.player_box_height],
            PlayerId.Orange: [1060, 730, self.player_box_width, self.player_box_height],
            PlayerId.Red: [1100, 730, self.player_box_width, self.player_box_height],
            PlayerId.Blue: [1140, 730, self.player_box_width, self.player_box_height]
        }
        ...
        self.game_log = ""
        self.game_log_target_rect = pygame.Rect(900, 335, 360, 120)  # Adjusted from 1140, 335, 560, 120
    ...
    def render_action_menu(self):
        ...
        for card in [DevelopmentCard.Knight, DevelopmentCard.VictoryPoint, DevelopmentCard.RoadBuilding,
            ...
            # Center the count text under the card
            text_width = card_count.get_width()
            text_x = card_pos[0] + (self.development_card_width - text_width) / 2
            self.screen.blit(card_count, (text_x, card_pos[1] + 105))
            card_pos[0] += shift

        self.screen.blit(self.ai_play_image, (30, self.screen.get_height() - 150))  # Adjusted to appear lower
            ...
            if self.game.must_respond_to_trade == False and len(self.active_other_player) > 0:
                if self.active_other_player[0] == player_id:
                    ...
                    pygame.draw.rect(self.screen, (0, 0, 0), 
                                   (box[0], box[1], box[2], box[3]), width=4)
    ...
    def render_harbour_exchange_images(self):
        self.screen.blit(self.harbour_images[Resource.Wood], (773, 670))
        self.screen.blit(self.harbour_images[Resource.Brick], (843, 670))
        self.screen.blit(self.harbour_images[Resource.Sheep], (913, 670))
        self.screen.blit(self.harbour_images[Resource.Wheat], (983, 670))
        self.screen.blit(self.harbour_images[Resource.Ore], (1053, 670))
        self.screen.blit(self.harbour_images[None], (1123, 670))
    ...
    def step_AI(self, deterministic=True):
        ...
        if isinstance(self.policies[players_go], str):
            show_message("It is currently a human player's turn.")
            return False
        else:
            ...
            if mouse_pos[0] > 914 and mouse_pos[0] < 1093 and mouse_pos[1] > 323 and mouse_pos[1] < 375:
                ...
                if mouse_click:
                    if len(self.active_development_res_boxes) == 1:
                        ...
                        show_message("No resources selected for year of plenty card")
                        continue
                    action = {
                        "type": ActionTypes.PlayDevelopmentCard,
                ...
                if mouse_click:
                    if len(self.active_development_res_boxes) == 0:
                        show_message("Must select a resource.")
                        continue
                    elif len(self.active_development_res_boxes) > 1:
                        show_message("Can only choose one resource with monopoly.")
                        continue
                    else:
                        ...
                        if mouse_click:
                            if res in self.active_development_res_boxes:
                                ...
                                if len(self.active_development_res_boxes) < 2:
                                    ...
                                    show_message("No development card involves more than 2 resources")
                        break
                        ...
                        if mouse_click:
                            if res in self.active_harbour_trade_res:
                                ...
                                if len(self.active_harbour_trade_res) == 0:
                                    ...
                                    show_message("Can only select one resource at a time.")
                        ...
                        if mouse_click:
                            if res in self.active_harbour:
                                ...
                                if len(self.active_harbour) == 0:
                                    ...
                                    show_message("Can only select one harbour at a time.")
            elif mouse_pos[0] > 1058 and mouse_pos[0] < 1203 and mouse_pos[1] > 899 and mouse_pos[1] < 934:
                if mouse_click:
                    if len(self.active_harbour_trade_res) == 0:
                        show_message("Need to select a resource to exchange.")
                    elif len(self.active_harbour_receive_res) == 0:
                        show_message("Need to select a resource to receive.")
                    else:
                        action = {
                ...
                for player_id in [PlayerId.White, PlayerId.Red, PlayerId.Orange, PlayerId.Blue]:
                    ...
                    if mouse_pos[0] > box[0] and mouse_pos[0] < (box[0] + box[2]) and mouse_pos[1] > box[1] and \
                        ...
                        if mouse_click:
                            if self.game.must_respond_to_trade:
                                show_message("Cannot alter a proposed trade. Accept or reject.")
                            else:
                                self.active_other_player = [player_id]
            elif mouse_pos[0] > 1457 and mouse_pos[0] < 1575 and mouse_pos[1] > 932 and mouse_pos[1] < 963:
                if mouse_click:
                    if self.game.must_respond_to_trade:
                        show_message("Cannot modify a proposed trade. Accept or reject")
                    else:
                        self.active_receive_res = []
                        self.active_trade_res = []
                ...
                if mouse_click:
                    if self.game.players_need_to_discard == False:
                        show_message("No one needs to discard resources at the moment")
                    else:
                        action = {
                ...
                if mouse_click:
                    if len(self.active_other_player) == 0:
                        show_message("You need to select a player to steal from")
                    else:
                        action = {
                ...
                if mouse_click:
                    if self.game.must_respond_to_trade:
                        ...
                        if len(self.active_trade_res) == 0:
                            show_message("Must choose resources to trade.")
                        elif len(self.active_receive_res) == 0:
                            show_message("Must propose resources to receive.")
                        elif len(self.active_other_player) == 0:
                            show_message("Must choose a player to propose the trade to.")
                        elif self.active_other_player[0] == players_go.id:
                            show_message("Cannot trade with yourself.")
                        else:
                            action = {
                ...
                for res in [Resource.Wood, Resource.Brick, Resource.Sheep, Resource.Wheat, Resource.Ore]:
                    ...
                    if mouse_pos[0] > box[0] and mouse_pos[0] < (box[0] + box[2]) and mouse_pos[1] > box[1] and \
                        ...
                        if mouse_click:
                            if self.game.must_respond_to_trade:
                                show_message("Cannot modify proposed trade. Accept or reject.")
                            else:
                                if len(self.active_trade_res) < self.game.max_trade_resources:
                                    self.active_trade_res.append(res)
                                else:
                                    show_message("Can only trade up to 4 resources at a time.")
